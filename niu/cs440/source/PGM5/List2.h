 ...
 *
 * Memo:  		Just curious, let me know how the doc is, (over done, ok, needs
 * 				work, etc)
 *
 * 					Thanks, 
 * 					Samir.
...
void List<T>::clear()
{
	vector<Node<T>*> arr(length);	// declares a dynamically 
	...
	arr.resize(0);					// deletes the array of pointers.
...
{
	vector<Node<T>*> new_ptr(length);	// creates array of pointers
...
void List<T>::traverse(void (&func)(T& element))
{
	...
	{
		(func)(currptr->entry);		// calls func passed by func pointer
